...
from exercise_db import EXERCISES, Exercise, lookup, normalize_code
...
def parse_payload(payload: dict[str, Any]) -> SessionInput:
    ...
    return SessionInput(
        ...
        sets=[
            SetRecord(
                # 正規化大小寫 + 連字號,避免同堂混寫導致 tonnage / PR 分裂計算
                exercise_code=normalize_code(str(s["exercise_code"])),
                sets=int(s["sets"]),
...
def _run_batch(args: argparse.Namespace) -> int:
    ...
    if parsed_sessions:
        ...
        if coach_section:
            summary_md = summary_md.rstrip("\n") + "\n\n" + coach_section
        category_section = render_studio_category_distribution(
            compute_studio_category_distribution(parsed_sessions)
        )
        if category_section:
            summary_md = summary_md.rstrip("\n") + "\n\n" + category_section
